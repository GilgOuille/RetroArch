...
- **`.sbi` (subchannel libcrypt, PSX PAL protégés)** : ce **n'est pas du contenu** → **court-circuite tout le pipeline** (un scan LOOSE d'un `.sbi` seul en ferait une entrée de playlist). Il doit vivre **à côté de l'image disque**, même nom de base : on le **copie dans `<système>/<ROM sans extension>/`** (le dossier d'extraction). Il est **aussi laissé à la racine du système** là où le download le dépose — c'est ce que voit `rom_library_task_entry_is_present` (« ✓ » sans cas particulier), et c'est déjà le bon emplacement si la ROM est un fichier simple (`.chd`). **Les deux ordres sont gérés** : SBI après la ROM (copie immédiate) et ROM après le SBI (rattrapage en fin de décompression, `rom_library_sbi_attach_to_dir`). SBI seul, sans ROM ⇒ **warning OSD** « download the ROM first », fichier conservé.
